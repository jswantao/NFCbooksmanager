    ...
    (config) => {
        if (config.method?.toLowerCase() === "get") {
            ...
            if (inFlight) {
                ...
                (config as Record<string, unknown>)._dedupResolve = resolveFn;
                (config as Record<string, unknown>)._dedupReject = rejectFn;
            }
        }
...
apiClient.interceptors.response.use(
    (response: AxiosResponse) => {
        ...
        if (config.method?.toLowerCase() === "get") {
            const key = getRequestKey(config);
            const resolveFn = (config as Record<string, unknown>)._dedupResolve as ((v: AxiosResponse) => void) | undefined;
            if (resolveFn) {
                resolveFn(response);
            }
    ...
    async (error: EnhancedAxiosError) => {
        ...
        if (config?.method?.toLowerCase() === "get") {
            const key = getRequestKey(config);
            const rejectFn = (config as Record<string, unknown>)._dedupReject as ((reason: unknown) => void) | undefined;
            if (rejectFn) {
                rejectFn(error);
            }
            inFlightRequests.delete(key);
