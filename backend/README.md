...
│   │   ├── schemas/          # Pydantic 请求/响应模型（按域拆分）
│   │   │   ├── common.py         # 通用模型 + AppSchema 基类
│   │   │   ├── nfc.py            # NFC 相关模型
│   │   │   ├── mapping.py        # 映射相关模型
│   │   │   ├── shelf.py          # 书架相关模型
│   │   │   ├── book.py           # 图书相关模型
...
| 债务项 | 说明 | 建议方案 | 状态 |
| -------- | ------ | ---------- | ------ |
| **巨型页面组件** | 15 个页面组件大小 800~1300 行（30K~52K 字节），业务逻辑、UI、样式混在一起 | 拆分为容器组件 + 展示组件，提取自定义 Hook 到 `src/hooks/` | 🔄 部分解决 |
| **巨型路由文件** | `nfc_bridge.py` (1481→850行) 和 `douban_service.py` (985→638行) 超长 | Jinja2 模板迁移（-633行），douban_parser.py 提取（-344行） | 🔄 部分解决 |
...
| **缺少 pyproject.toml** | 后端无 `pyproject.toml` | 迁移至 `pyproject.toml` 统一管理 | ⏳ 待处理 |
| **缺少 `.env.example` 文档** | 未列出所有可配置项和默认值 | 补充完整的配置项说明 | ⏳ 待处理 |
| **`tsc -b` 构建未通过** | `icons.ts` 动态导入类型不兼容等 | 修复类型定义 | ⏳ 待处理 |
...
| 功能 | 目标 | 优先级理由 |
| ------ | ------ | ------------ |
| **自动化测试体系** | 后端 `pytest` 覆盖核心 API 和服务层；前端 `Vitest + React Testing Library` 覆盖关键组件和 Hook | 无测试即无法安全重构，是所有后续工作的前提 |
| **Alembic 数据库迁移** | 引入 Alembic，生成初始迁移脚本，建立增量迁移流程 | 当前 `create_all()` 无法应对生产环境的 schema 变更 |
| **CI/CD 流水线** | GitHub Actions：lint → type-check → test → build → deploy | 保障代码质量和自动化部署 |

### P1 — 系统可靠性提升（v2.2）

| 功能 | 目标 | 状态 |
| ------ | ------ | ------ |
| **Jinja2 模板迁移** | 将 NFC bridge 中 750 行 HTML/Python 混合代码迁移至 Jinja2 模板（5 个模板文件，`nfc_bridge.py` 减少 633 行） | ✅ 已完成 |
| **日志系统统一** | 全项目统一使用 `loguru`，移除标准 `logging` 依赖（5 个模块），替换 23 处 `print()` | ✅ 已完成 |
| **统一异步数据库访问** | 通过 `run_sync_db_block()` 将同步 DB 操作放入线程池（nfc_bridge 10 处全部修复） | 🔄 部分完成 |
| **依赖注入重构** | 用 FastAPI `Depends()` 统一管理 `DoubanService`、数据库会话等服务依赖 | ⏳ 计划中 |
| **豆瓣服务优化** | SimpleCache → TTLCache（maxsize+TTL）、asyncio.Lock 并发安全、限流时钟 → monotonic() | ✅ 已完成 |
| **配置管理增强** | 完善 `.env.example`，迁移至 `pyproject.toml`，凭证加密存储 | ⏳ 计划中 |

### P2 — 功能拓展（v2.3）

| 功能 | 目标 | 优先级理由 |
| ------ | ------ | ------------ |
| **图书标签打印** | 生成包含 ISBN、书名、位置编码的标签图片，支持打印机输出 | 提升实体书架管理的效率 |
| **阅读统计增强** | 阅读进度追踪、读书笔记关联、年度阅读报告 | 从"管理"升级为"阅读伴侣" |
| **多用户支持** | 家庭成员账户切换，个人书架隔离，共享书架的权限控制 | 覆盖家庭共用场景 |
| **数据导出/导入** | 支持 JSON/CSV 格式的完整数据导出和导入（含书架、映射关系） | 数据迁移和备份需求 |
| **移动端适配优化** | NFC 操作页面 PWA 独立窗口，扫描历史记录，离线缓存增强 | 手机端是 NFC 操作的主要场景 |
| **PostgreSQL 支持** | 除 SQLite 外正式支持 PostgreSQL，利用 JSONB、全文搜索等特性 | 为大规模部署和多用户场景做好准备 |

### P3 — 体验与智能化（v2.4+）

| 功能 | 目标 | 优先级理由 |
| ------ | ------ | ------------ |
| **AI 图书推荐** | 基于馆藏和阅读记录，使用 LLM 生成个性化推荐 | 差异化功能，提升用户价值 |
| **OCR 识别录入** | 拍照识别图书封面或 ISBN 条形码，自动录入 | 降低手动录入门槛 |
| **图书社区集成** | 与豆瓣、Goodreads 等的评分/评论同步，社交分享 | 丰富图书元数据来源 |
| **语音搜索** | 支持中文语音输入搜索馆藏图书 | NFC 操作场景的无障碍补充 |
| **桌面客户端** | 基于 Tauri 的桌面应用，支持系统托盘、全局快捷键、离线使用 | 提升桌面使用体验 |
| **i18n 国际化** | 中/英/日多语言界面支持 | 拓展海外用户场景 |
