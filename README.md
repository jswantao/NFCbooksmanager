...
基于三级模式架构的智能书房管理系统。当前版本 v2.4.0，已集成 n8n AI 工作流、智能录入、Storybook 组件库。
...
- 🤖 **n8n AI 智能助手**：自然语言搜索馆藏图书 (n8n 工作流编排后端 API)、智能录入与信息补全 (多源元数据查询 + 字段自动补全)
- 📚 **智能录入增强**：多模态 ISBN 识别、豆瓣/Google Books/OpenLibrary 三级降级查询、Google Books API 集成（API Key 可选）、书架选择与来源标记
- 📖 **Storybook 组件库**：交互式组件文档、Controls 面板、可访问性审计、视觉回归测试
...
- 🖼️ **封面管理**：本地封面缓存（7 天有效期）+ 本地封面上传（替换/删除）+ 图片代理
- 📝 **日志与审计**：TypeScript 严格模式 + Pydantic v2 泛型 + 100+ API 路由
- ⚡ **性能优化**：React.lazy 代码分割、图片懒加载、虚拟滚动表格、请求去重
- 📝 **完整日志系统**：loguru 多通道输出、文件轮转、自动压缩
- 🔄 **乐观更新**：关键操作立即更新 UI，失败时自动回滚
- 📝 **日志与审计**：TypeScript 严格模式，loguru 全项目统一 + 操作审计日志 + 请求级中间件
...
系统在智能录入时会按以下顺序查询图书信息：

```
1. 豆瓣 (Douban)        ← Cookie 有效时优先，中文图书元数据最完整
   ↓ 失败
2. Google Books API     ← 官方 API，配额稳定，支持 API Key 提升额度
   ↓ 失败
3. OpenLibrary          ← 免费开放数据源，无需认证，最后兜底
   ↓ 失败
4. 手动录入引导          ← 所有数据源不可用时，提示用户手动填写
```

**Google Books API 配置（可选）：**

1. 访问 [Google Cloud Console](https://console.cloud.google.com/)
2. 创建项目 → 启用 Books API → 创建 API 密钥
3. 在 `backend/.env` 中设置：
   ```bash
...
> **注意**：不配置 API Key 也可使用（匿名配额 ~100 次/天），配置后提升至 1000 次/天。中国大陆可能需要代理才能访问 Google API。
...
│   │   │   ├── books.py          # 图书管理（含统一操作入口 + Dify 同步触发）
│   │   │   ├── physical_shelves.py # 物理书架管理
│   │   │   ├── admin.py          # 管理统计（仪表盘 + 操作日志 + 同步日志）
│   │   │   ├── images.py         # 图片代理/缓存 + 本地上传
│   │   │   ├── config_api.py     # Cookie 配置
│   │   │   ├── import_api.py     # 批量导入
│   │   │   ├── backup.py         # 备份/恢复/WebDAV 云同步（14 个端点）
│   │   │   ├── chat.py           # AI 聊天/知识库搜索/知识库同步/重建（10 个端点）
│   │   │   ├── smart_entry.py    # 智能录入/OCR/ISBN查询/信息补全（8 个端点）
│   │   │   └── __init__.py
│   │   ├── core/             # 核心配置
...
│   │   ├── schemas/          # Pydantic 请求/响应模型（按域拆分，12 个模块）
...
│   │   │   ├── chat.py           # AI 聊天/知识库同步模型
│   │   │   ├── smart_entry.py    # 智能录入/信息补全模型（16 个 Schema）
...
│   │   │   ├── dify_sync_service.py # Dify 知识库同步（合并文档 + 速率限制 + 防抖重建）
│   │   │   ├── chat_service.py   # AI 聊天服务（n-gram 搜索 + 相似推荐 + 知识库导出）
│   │   │   ├── smart_entry_service.py # 智能录入（多源 ISBN 查询 + 字段补全 + 条形码扫描）
│   │   │   └── kb_builder_service.py # 本地知识库构建（分类推断 + 富化文本 + 质量报告）
│   │   ├── templates/        # Jinja2 模板
│   │   │   ├── error.html        # 错误页面
...
│   ├── dify/                 # Dify 云端配置
│   │   ├── smart-entry-chatflow.yml   # 智能录入 Chatflow DSL（37 节点工作流）
│   │   ├── smart-entry-tools.json     # 智能录入 API 工具定义（OpenAPI 3.1）
│   │   ├── smart-entry-chatflow-guide.md # Chatflow 部署配置指南
│   │   ├── node-prompts.md            # LLM 节点提示词参考（11 个节点）
│   │   ├── knowledge-base-precision-analysis.md # 知识库精准度分析报告
│   │   ├── knowledge-base-rich.md     # 已导出的富化知识库文件（102 本, 119 KB）
│   │   └── (smart-entry-agent-prompt.md + tools.json + knowledge-base-import-guide.md 等)
│   ├── logs/                 # 日志文件目录（loguru 多通道：app.log + app.error.log）
│   ├── requirements.txt      # Python 依赖
│   ├── pyproject.toml        # 项目配置（元数据 + 依赖 + ruff/mypy/pytest）
│   └── .env.example          # 环境变量示例（含 Dify + 备份和 WebDAV 配置）
│
├── frontend/
│   ├── public/               # 静态资源
...
│   │   │   ├── BookEditor.tsx        # 统一图书编辑（双模式路由 + 本地上传封面）
...
│   │   │   ├── Dashboard.tsx         # 管理仪表盘（多图表 + 知识库同步状态卡片）
│   │   │   ├── BackupManager.tsx     # 备份管理（4 标签页：即时/列表/自动/云同步）
│   │   │   ├── BackupRestore.tsx     # 恢复向导（3 步：预览→冲突检测→结果）
│   │   │   ├── ChatAssistant.tsx     # AI 助手（Dify Chatflow iframe 嵌入）
│   │   │   └── SmartEntry.tsx        # 智能录入（3 Tab：单本录入/信息补全/批量导入）
│   │   ├── services/         # API 服务层
...
- **20 个页面组件**：含 Dify AI 助手和智能录入，每个页面功能完整（React.lazy）
- **4 个图表组件**：统一的加载/空状态/错误处理
- **15 个自定义 Hook**：容器/展示分离，纯逻辑与 UI 渲染职责清晰
- **24 个后端服务**：含 Dify 同步、AI 聊天、智能录入、知识库构建等
...
| 功能                        | 目标                                                                             | 状态      |
| --------------------------- | -------------------------------------------------------------------------------- | --------- |
| **n8n AI 智能助手**  | 基于 Dify Chatflow 的自然语言图书搜索、相似推荐、知识库 RAG 检索                 | ✅ 已完成 |
| **智能录入与补全**    | 多源元数据查询 + 字段自动补全（豆瓣/Google Books/OpenLibrary）+ 自动字段补全 | ✅ 已完成 |
| **知识库本地构建**    | 规则引擎分类推断 + 富化文本生成 (3x 质量提升) + 合并文档策略上传                 | ✅ 已完成 |
| **Dify 速率限制适配** | 令牌桶 8 次/分钟 + 合并文档 (2 API 调用全量) + 防抖增量重建 30s                  | ✅ 已完成 |
| **图书封面上传**      | 本地封面图片上传/替换/删除，getBestCoverUrl 优先级策略                           | ✅ 已完成 |
| **导航响应式重构**    | 减少顶级导航项 (9→7)、xl 断点 (1200px)、菜单项 margin 优化                      | ✅ 已完成 |
...
| 功能                      | 目标                                                      | 状态      |
| ------------------------- | --------------------------------------------------------- | --------- |
| **AI 图书推荐增强** | 基于阅读记录的个性化推荐 + 馆藏冷启动推荐                 | ⏳ 待处理 |
...
Copyright (c) 2024-2026 jswantao
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v2.4.1** | 2026-05 | Google Books API 集成：适配器 + 多数据源优先级调度 |
| | | 数据源降级链：豆瓣 → Google Books → OpenLibrary → 手动录入 |
| | | GoogleBooksService (缓存/限流/统计)、前端数据源 Tag 提示 |
| **v2.4.0** | 2026-05 | Dify 全面迁移至 n8n：智能录入 + 书房助手双工作流 |
| | | 智能录入增强：来源标记 (smart_entry)、书架选择、图片代理 |
| | | AI 对话 Markdown 渲染 (react-markdown, 15 种格式元素) |
| | | Storybook 组件库文档系统 (6 组件, a11y + interactions) |
| | | Service Worker 移除、Dify 配置清理、DifySyncLog 模型移除 |
| | | GET /api/books/ 新端点、POST /smart-entry/search 新端点 |
| | | 修复：导航重叠、n8n 响应格式适配、ApiResponse[None] 验证 |
| **v2.3.0** | 2026-04 | 智能录入 Agent (Dify Chatflow) + 知识库富化构建 (3x) |
| | | 速率限制适配 (令牌桶 8/min + 合并文档策略) |
| | | 图书编辑器本地封面上传 + BookEditor 双模式路由统一 |
| | | 前端全页面审计修复 (18 页) + CLAUDE.md 开发规范 |
| **v2.2.0** | 2026-03 | 备份与恢复 (WebDAV + 冲突检测) + NFC 任务持久化 |
| | | Jinja2 模板迁移 + loguru 全项目统一 |
| | | PWA Service Worker + 主题系统 (5 套) |
